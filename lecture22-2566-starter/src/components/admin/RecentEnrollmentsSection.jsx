      ...
      {!loadingRecentEnrollments &&
        ...
        recentEnrollments.map((enroll) => (
          <Group spacing="xs" key={enroll.id}>
            <Text fw="bold" color="dimmed">{enroll.student.studentId} - {enroll.student.firstName}</Text>
            <Text color="dimmed">👉</Text>
            <Text fw="bold" color="dimmed">{enroll.course.courseNo} - {enroll.course.title}</Text>
            <Text color="dimmed" ml="auto">{formatDate(enroll.createdAt)}</Text>
          </Group>
        ))}
    </Paper>
  );
